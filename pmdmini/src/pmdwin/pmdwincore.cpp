...
/* PS3 (PowerPC) is big-endian; PMD .M data is little-endian (PC-98/x86).
 * All multi-byte reads from mmlbuf/musdata must go through these helpers. */
...
//					bx = open_work.rhyadr = &open_work.mmlbuf[*bx];
					bx = open_work.rhyadr = &open_work.mmlbuf[read_le16(&open_work.radtbl[al])];
					goto rhyms00;
				}
...
uint8_t * PMDWIN::rhythmon(QQ *qq, uint8_t *bx, int32_t al, int32_t *result)
{
	...
	*result = 1;
	...
	if(open_work.kp_rhythm_flag) {
		...
		/* リズム keyon カウンタ更新 */
		if(keyon_bits & 0x01) open_work.rshot_bd++;
		if(keyon_bits & 0x02) open_work.rshot_sd++;
		if(keyon_bits & 0x04) open_work.rshot_sym++;
		if(keyon_bits & 0x08) open_work.rshot_hh++;
		if(keyon_bits & 0x10) open_work.rshot_tom++;
		if(keyon_bits & 0x20) open_work.rshot_rim++;
	}
...
uint8_t * PMDWIN::pcmrepeat_set8(QQ *qq, uint8_t * si)
{
	...
	loop_start = (int16_t)read_le16(si);
	si += 2;
	loop_end = (int16_t)read_le16(si);
	si += 2;
	release_start = (int16_t)read_le16(si);
...
uint8_t * PMDWIN::commands(QQ *qq, uint8_t *si)
{
	...
	switch(al) {
		...
		case 0xfa : qq->detune = (int16_t)read_le16(si); si+=2; break;
		...
		case 0xd5 : qq->detune += (int16_t)read_le16(si); si+=2; break;
		//
...
void PMDWIN::opnint_start(void)
{
	...
	pmdwork.rhydmy = 255;
	/* BE fix: rhydmy is cast to (uint8_t*) and first byte must be 0xFF */
	*(uint8_t *)&pmdwork.rhydmy = 0xFF;
	data_init();
	opn_init();
...
{
	int32_t			i;
	int32_t			pofs;	/* byte offset into mmlbuf (replaces uint16_t *p) */
	uint16_t		pval;	/* LE16 value read via helper */

	open_work.x68_flg = *(open_work.mmlbuf-1);

	//２．６追加分  — read LE16 offset
	if(read_le16(open_work.mmlbuf) != 2*(max_part2+1)) {
		open_work.prgdat_adr = open_work.mmlbuf + read_le16(&open_work.mmlbuf[2*(max_part2+1)]);
		open_work.prg_flg = 1;
	} else {
		open_work.prg_flg = 0;
	}

	pofs = 0;	/* byte offset, increments by 2 per part */

	//	Part 0,1,2,3,4,5(FM1?6)の時
	for(i = 0; i < 6; i++) {
		pval = read_le16(&open_work.mmlbuf[pofs]);
		if(open_work.mmlbuf[pval] == 0x80) {		//先頭が80hなら演奏しない
			FMPart[i].address = NULL;
		} else {
			FMPart[i].address = &open_work.mmlbuf[pval];
		}
	...
	for(i = 0; i < 3; i++) {
		...
		if(open_work.mmlbuf[pval] == 0x80) {		//先頭が80hなら演奏しない
			...
			SSGPart[i].address = &open_work.mmlbuf[pval];
		}
	...
	if(open_work.mmlbuf[pval] == 0x80) {		//先頭が80hなら演奏しない
		...
		ADPCMPart.address = &open_work.mmlbuf[pval];
	}
	...
	if(open_work.mmlbuf[pval] == 0x80) {		//先頭が80hなら演奏しない
		...
		RhythmPart.address = &open_work.mmlbuf[pval];
	}
	...
	pval = read_le16(&open_work.mmlbuf[pofs]);
	open_work.radtbl = (uint16_t *)&open_work.mmlbuf[pval];
	open_work.rhyadr = (uint8_t *)&pmdwork.rhydmy;
}
...
int32_t PMDWIN::ppc_load3(uint8_t *pcmdata, int32_t size)
{
	...
	if(strncmp((char *)pcmdata, PVIHeader, sizeof(PVIHeader)-1) == 0 &&
		...
		//--------------------------------------------------------------------
		 ...
		 for(i = 0; i < 128; i++) {
			if(read_le16(&pcmdata[18+i*4]) == 0) {
				pcmends.pcmadrs[i][0] = pcmdata[16+i*4];
				pcmends.pcmadrs[i][1] = 0;
			} else {
				pcmends.pcmadrs[i][0] = read_le16(&pcmdata[16+i*4])+0x26;
				pcmends.pcmadrs[i][1] = read_le16(&pcmdata[18+i*4])+0x26;
			}
...
void WINAPI PMDWIN::reset_opna(void)
{
	opna->Reset();
	opna->SetRate(OPNAClock, open_work.rate, open_work.fmcalc55k);
}
...
char* WINAPI PMDWIN::_getmemo(char *dest, uint8_t *musdata, int32_t size, int32_t al)
{
	...
	if(maxsize < (int32_t)read_le16(&mmlbuf[0x18]) - 4 + 3) {
		*dest = '\0';			// 	曲データが不正
		return NULL;
	}

	si = &mmlbuf[read_le16(&mmlbuf[0x18]) - 4];
	if(*(si + 2) != 0x40) {
