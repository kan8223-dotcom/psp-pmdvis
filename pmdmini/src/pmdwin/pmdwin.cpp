...
API_ATTRIBUTE void WINAPI reset_opna(void)
{
	pmdwin->reset_opna();
}
...
API_ATTRIBUTE uint32_t WINAPI get_opna_sample_rate(void)
{
	if (pmdwin) {
		FM::OPNAW* o = pmdwin->getopna();
		if (o) return o->get_sample_rate();
	}
	return 0;
}
 ...
 * ============================================================ */
API_ATTRIBUTE void WINAPI setpcmrate2(int rate)
{
	if (pmdwin2) pmdwin2->setpcmrate(rate);
}

API_ATTRIBUTE int WINAPI music_load2_inst2(uint8_t *musdata, int size)
{
	if (!pmdwin2) return -1;
	return pmdwin2->music_load2(musdata, size);
}

API_ATTRIBUTE void WINAPI music_start2(void)
{
	if (pmdwin2) pmdwin2->music_start();
}

API_ATTRIBUTE void WINAPI music_stop2(void)
{
	if (pmdwin2) pmdwin2->music_stop();
}

API_ATTRIBUTE void WINAPI getpcmdata2(short *buf, int nsamples)
{
	if (pmdwin2) pmdwin2->getpcmdata(buf, nsamples);
}

API_ATTRIBUTE int WINAPI getloopcount2(void)
{
	if (!pmdwin2) return 0;
	return pmdwin2->getloopcount();
}

API_ATTRIBUTE void WINAPI setrhythmwithssgeffect2(bool value)
{
	if (pmdwin2) pmdwin2->setrhythmwithssgeffect(value);
}
